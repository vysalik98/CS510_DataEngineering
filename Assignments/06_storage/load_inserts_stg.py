...
def createTable(conn):
    with conn.cursor() as cursor:
        cursor.execute(
            ...
            DROP TABLE IF EXISTS {TableName}_staging;
            CREATE UNLOGGED TABLE {TableName}_staging (
				CensusTract         NUMERIC,
				State               TEXT,
				County              TEXT,
				TotalPop            INTEGER,
...
def load(conn, icmdlist):
    with conn.cursor() as cursor:
        ...
        for cmd in icmdlist:
            cursor.execute(cmd.replace(TableName, f"{TableName}_staging"))
...
def add_constraint(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"INSERT INTO {TableName} SELECT * FROM {TableName}_staging;")
        ...
        cursor.execute(f"DROP TABLE {TableName}_staging;")
